using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace totalsale_report
{
    public class cReportSaleCoReportScr
    {
        public string DocuDate { get; set; }
        public string InvNo { get; set; }
        public string CustPONo { get; set; }
        public string CustCode { get; set; }
        public string CustName { get; set; }
        public string ProductCode { get; set; }
        public string Product { get; set; }
        public string GoodGroupCode { get; set; }
        public string GoodGroupName { get; set; }
        public string GoodCode { get; set; }
        public string Model { get; set; }
        public string GoodName { get; set; }
        public string Amount { get; set; }
        public string RF { get; set; }
        public string RentAmount { get; set; }
        public string TotalPrice { get; set; }
        public string RentCom { get; set; }
        public string NetCom { get; set; }
        public string RentOther { get; set; }
        public string NetPrice { get; set; }
        public string NetRF_B { get; set; }
        public string NetRF_P { get; set; }
        public string NetCutSale { get; set; }
        public string NetItems { get; set; }
        public string GoodPattnCode { get; set; }
        public string GoodPattnName { get; set; }
        public string GoodColorCode { get; set; }
        public string GoodColorName { get; set; }
        public string CustTypeCode { get; set; }
        public string CustTypeName { get; set; }
        public string sRemark { get; set; }
        public string GoodClassCode { get; set; }
        public string GoodClassName { get; set; }
        public string EmpCode { get; set; }
        public string SaleName { get; set; }
        public string Trs_P { get; set; }
        public string JobsCode { get; set; }
        public string JobsName { get; set; }
        public string ID { get; set; }
        public string Remark { get; set; }
    }
}