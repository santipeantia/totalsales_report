<%@ Page Title="" Language="C#" MasterPageFile="~/total_sale.master" AutoEventWireup="true" CodeBehind="ampelite-reports-sales-co.aspx.cs" Inherits="totalsale_report.xreporting.ampelite_reports_sales_co" %>
    ...
    <section class="content-header">
        ...
        <script>
            ...
            function getSyncData() {
                $.ajax({
                    url: '../../xreporting/reporting_srv.asmx/GetSyncDataSaleCo',
                    method: 'post',
                    datatype: 'json',
                    beforeSend: function () {
                        $('#overlay').show();
            ...
            function getReportSalesCoReport(sdate, edate, soption, stype, search) {
                $.ajax({
                    url: '../../xreporting/reporting_srv.asmx/GetReportSalesCoReport',
                    method: 'post',
                    data: {
                        sdate: sdate,
                        edate: edate,
                    ...
                    datatype: 'json',
                    beforeSend: function () {
                        $("#tblSalesCoReport tr td").remove();
                        $("#divloader").show();
                    },
                    success: function (data) {
                        var table;
                        table = $('#tblSalesCoReport').DataTable();
                        table.clear();
                        ...
                        if (data != '') {
                            $.each(data, function (i, item) {
                                table.row.add([data[i].DocuDate, data[i].InvNo, data[i].CustPONo, data[i].CustCode, data[i].CustName, data[i].ProductCode, data[i].Product
                                    , data[i].GoodGroupCode, data[i].GoodGroupName, data[i].GoodCode, data[i].Model, data[i].GoodName, data[i].pLength, data[i].pQuantity
                                    , data[i].Amount, data[i].RF, data[i].RentAmount, data[i].TotalPrice, data[i].RentCom, data[i].NetCom, data[i].RentOther, data[i].NetPrice
                                    , data[i].NetRF_B, data[i].NetRF_P, data[i].NetCutSale, data[i].NetItem, data[i].GoodPattnCode, data[i].GoodPattnName, data[i].GoodColorCode
                                    , data[i].GoodColorName, data[i].CustTypeCode, data[i].CustTypeName, data[i].sRemark, data[i].GoodClassCode, data[i].GoodClassName, data[i].EmpCode
                                    , data[i].SaleName, data[i].Trs_P, data[i].JobsCode, data[i].JobsName]);
                            });
                        }
                        table.draw();
                        $("#divloader").hide();
                ...
                <div class="">
                    <div class="box box-solid">
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblSalesCoReport" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
